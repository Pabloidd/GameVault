<script>
  ...
  import { validationRules, normalize } from '$lib/utils/validation';
  ...
  async function handleSubmit(e) {
    ...
    const validationError = validationRules.country(name);
    if (validationError) {
    ...
    try {
      if (isEdit) {
        await tableStore.update({ name: normalizedName });
      } else {
        await tableStore.create({ name: normalizedName });
      }
    } catch (err) {
      error = mapError(err);
    } finally {
